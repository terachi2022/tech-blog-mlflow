...
Tracking Serverを構築し、Iris分類を記録して、MLflow UIで2つのRunを比較します。
...
- 修正: `uv add "mlflow=={{MLFLOW_VERSION}}" scikit-learn`へ統合
...
Apple M5 Max上の当該Runでは、MLX-LMは約97〜101 Token/秒、Peak Memoryは5.379〜5.613 GBでした。max_tokens増加は切断を防ぎましたが、品質向上を保証しません。この観測は当該Machine、Model、Promptに限定します。
...
- SQLite構成はローカル学習向けで、本番運用では永続性と同時接続を別途設計する
- Tracking Serverを起動してから、同じTracking URIで`train.py`を実行する

## まとめ

MLflowでIris分類をRunとして記録し、UIで条件と結果を比較できました。
