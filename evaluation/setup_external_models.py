"""STEP-α-3: External ModelをMLflowへ登録するCLI。"""
...
def main() -> None:
    ...
    path = output_dir / f"external_models_alpha_3_{mode}_{timestamp}.json"
    path.write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    print("=" * 60)
    print("STEP-α-3 MLflow External Model Setup")
    print("=" * 60)
    print("Mode       :", result["mode"])
    print("Experiment :", result["experiment_name"])
    print("Models     :", len(result.get("registrations", result["plans"])))
    if not args.dry_run:
        for item in result["registrations"]:
            print(f"{item['role'].title():10}: {item['model_uri']}")
