...
class PromptV3Test(unittest.TestCase):
    ...
    def test_generation_settings_are_fixed(
        ...
        expected_fragments = (
            ...
            'PROMPT_VERSION: Final = "article-v3.5.1"',
            '"generation-v3.5.1"',
            "MAX_TOKENS - 64",
...
def main():
    try:
        ...
        raise RuntimeError("Tracking ServerとURIを確認") from exc
...
class EvaluatePromptV3Test(unittest.TestCase):
    def test_build_evaluation_command(
        ...
        self.assertIn(
            "evaluation.evaluate_combined_v2_3",
            command,
        )
        self.assertIn("prompt-v3.5.1", command)
        self.assertIn(
            EXPECTED_PROMPT_VERSION,
            command,
        )
